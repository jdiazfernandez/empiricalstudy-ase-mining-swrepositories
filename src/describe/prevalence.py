...
def main() -> int:
    ...
    for p in args.raw:
        ...
        if not path.exists():
            print(
                f"ERROR: mining output file does not exist: {p}\n"
                "Run this first:\n"
    ...
    if not recs:
        print(
            "ERROR: mining output files contain no records.\n"
            "Run this first:\n"
            "  python src/mine/mine_harness.py --stratum all",
            file=sys.stderr,
        )
    ...
    cloned = [r for r in recs if r.get("status") == "cloned"]
    if not cloned:
        strata = sorted({str(r.get("stratum")) for r in recs})
        print(
            "ERROR: no cloned repositories found in the supplied files.\n"
            f"Strata found: {', '.join(strata)}\n"
            "The files contain mining results, but every repository is still "
            "marked as unavailable or api_fallback.\n"
            "Complete or retry mining before calculating prevalence:\n"
            "  python src/mine/mine_harness.py --stratum all",
            file=sys.stderr,
        )
        return 2
