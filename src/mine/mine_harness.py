...
def main() -> int:
    ...
    ap.add_argument("--stratum", default=None, help="restrict to one stratum (P, G1, G2, G3)")
    ...
    if args.stratum:
        ...
        if not rows:
            print(f"no rows with stratum={args.stratum!r} in {args.frame}", file=sys.stderr)
            return 2
    if args.limit:
        rows = rows[: args.limit]

    prefix = args.out_prefix or f"harness_{args.stratum or 'all'}"
    out_json = ROOT / "data" / f"{prefix}_raw.json"
    ledger = ROOT / "data" / f"{prefix}_ledger.csv"
    ckpt = ROOT / "data" / f"{prefix}_checkpoint.jsonl"

    CLONES.mkdir(parents=True, exist_ok=True)
    RULEFILES.mkdir(parents=True, exist_ok=True)

    # Resume: anything already checkpointed is not re-cloned (C-v2-1 idempotent).
    done: dict[str, dict] = {}
    if ckpt.exists():
        ...
        print(f"resuming: {len(done):,} repositories already done in {ckpt.name}")

    todo = [r for r in rows if r["full_name"] not in done]
    print(f"frame {pathlib.Path(args.frame).name} | stratum {args.stratum or 'all'} | "
            ...
            for f in as_completed(futs):
                try:
                    ...
                    rec = {"repo_id": -1, "full_name": name, "status": "unavailable",
                           ...
                           "resolved_full_name": None, "stratum": args.stratum}
                results.append(rec)
                n_done += 1
                with _lock:
                ...
                if n_done % 25 == 0 or n_done == len(todo):
                    ...
                    stuck = sum(1 for r in results if r.get("clone_removed") is False)
